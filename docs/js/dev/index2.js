...
function spollers() {
  ...
  if (spollersArray.length > 0) {
    let playSpollerVideo = function(spollerBlock) {
    ...
    }, resetSpollerVideo = function(spollerBlock) {
      const video = spollerBlock.querySelector("video[data-spollers-video]");
      if (!video) return;
      video.pause();
      video.currentTime = 0;
    }, initSpollers = function(spollersArray2, matchMedia2 = false) {
      ...
      if (el.closest("summary") && el.closest("[data-fls-spollers]")) {
        ...
        if (el.closest("[data-fls-spollers]").classList.contains("--spoller-init")) {
          ...
          if (!spollersBlock.querySelectorAll(".--slide").length) {
            const isOpening = !spollerBlock.open;
          ...
          spollersClose.forEach((spollerClose) => {
            ...
            if (spollersBlock.classList.contains("--spoller-init")) {
              ...
              setTimeout(() => {
                ...
                resetSpollerVideo(spollerCloseBlock);
              }, spollerSpeed);
            }
          });
        }
      ...
      if (spollerActiveBlock && !spollersBlock.querySelectorAll(".--slide").length) {
        ...
        setTimeout(() => {
          resetSpollerVideo(spollerActiveBlock);
...
const marquee = () => {
  ...
  const onWindowWidthResize = (cb) => {
    ...
    window.addEventListener("resize", debounce2(50, handleResize));
    handleResize();
  };
    ...
    document.querySelectorAll("[data-gsap]").forEach((section) => {
      ...
      gsapWithCSS.to(section, {
        ...
        scrollTrigger: {
          ...
          scrub: 0.6,
          // для более гладкой анимации вместо true
          invalidateOnRefresh: true
        }
      });
      const trustImg = section.querySelector(".trust__img img");
      if (trustImg) {
        gsapWithCSS.to(trustImg, {
          y: 0,
          duration: 2,
          ease: "none",
          scrollTrigger: {
