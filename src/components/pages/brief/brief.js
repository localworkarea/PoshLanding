...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function applyBranching() {
    ...
    // Пересчёт общего кол-ва (НО без первого шага)
  ...
  function showStep(index) {
    ...
    // Кнопки
    if (current === 0) {
      // первый шаг — только кнопка ДАЛЕЕ
      ...
      // последний шаг — кнопка отправить
    ...
    // Обновление прогресса, начиная со 2-го шага
    if (current > 0) {
      progress.style.display = "flex";
      const currentNumber = current; // step #2 = 1
  ...
  firstStep.addEventListener("change", e => {
    ...
    // остаёмся на первом шаге, никакого автоматического перехода!
    showStep(0);
  });
  ...
  btnNext.addEventListener("click", () => {
    ...
    if (!branchSelected) {
      // позволяем браузеру показать required
  ...
  // ============= ресет после отправки (из form.js) =============
  document.addEventListener("briefResetSteps", () => {
    current = 0;
